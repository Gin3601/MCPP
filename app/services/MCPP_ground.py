...
def run(images: dict):
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.error("MCPP_ground invalid response: %s", result)
            raise ServiceError("上游返回格式异常")

        outputs = data.get("outputs") or []
        if outputs:
            logger.info("MCPP_ground success (sync)")
            return {"status": "success", "output": outputs[0], "mode": "sync"}

        # 同步失败 → 异步兜底
        result_url = (data.get("urls") or {}).get("get")
        if not result_url:
            logger.error("MCPP_ground no outputs and no result url: %s", result)
        ...
        if not foutputs:
            logger.error("MCPP_ground async done but still no outputs: %s", final)
            raise ServiceError("模型未返回结果")

        logger.info("MCPP_ground success (async fallback)")
        return {"status": "success", "output": foutputs[0], "mode": "async"}

    except ServiceError:
        raise
    except Exception:
        logger.exception("MCPP_ground crashed")
