...
logger = get_logger("MCPP_information")
...
def run(images: dict):
    logger.info("MCPP_information start")
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.error("MCPP_information invalid response: %s", result)
            raise ServiceError("上游返回格式异常")

        outputs = data.get("outputs") or []
        if outputs:
            logger.info("MCPP_information success (sync)")
            return {"status": "success", "output": outputs[0], "mode": "sync"}

        # 同步没拿到 outputs：异步兜底
        result_url = (data.get("urls") or {}).get("get")
        if not result_url:
            logger.error("MCPP_information no outputs and no result url: %s", result)
        ...
        if not foutputs:
            logger.error("MCPP_information async done but still no outputs: %s", final)
            raise ServiceError("模型未返回结果")

        logger.info("MCPP_information success (async fallback)")
        return {"status": "success", "output": foutputs[0], "mode": "async"}

    except ServiceError:
        raise
    except Exception:
        logger.exception("MCPP_information crashed")
        raise ServiceError("MCPP_information internal error")
